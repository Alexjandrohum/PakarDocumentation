...
public class ComentarioDAO {
    ...
    public List<ComentarioDTO> getComentarios(int idTienda, Connection con) {
        ...
        String query
                ...
                + "	c.comentario \n"
                + "FROM aps.comentario c WITH(NOLOCK) \n"
                + "INNER JOIN aps.usuario u WITH(NOLOCK) ON c.id_usuario = u.id_usuario \n"
                + "INNER JOIN aps.vw_empleado e WITH(NOLOCK) ON u.numero_control = e.numero_control \n"
    ...
    public boolean registraComentario(ComentarioDTO comentario, String token, int idNotificacion, Connection con) {
        ...
        PreparedStatement ps = null;
        try {
            query = "INSERT INTO aps.comentario \n"
                    + "SELECT ? AS id_tienda, (SELECT id_usuario FROM aps.usuario WHERE token_usuario = ?) AS id_usuario, ? AS comentario, GETDATE() AS fecha, ? AS id_comentario_padre, " + (idNotificacion == 0 ? "NULL" : idNotificacion) + " AS id_notificacion ";
            ps = con.prepareStatement(query);
    ...
    public int registraNotificacion(int idTipoNotificacion, Connection con) {
        ...
        PreparedStatement ps = null;
        try {
            query = "INSERT INTO aps.notificacion \n"
    ...
    public boolean isSupervisor(int idTienda, String token, Connection con) {
        ...
        try {
            query
                    ...
                    + "     FROM aps.vw_empleado \n"
                    + "     WHERE numero_control = (SELECT numero_control FROM aps.usuario WHERE token_usuario = ?) \n"
