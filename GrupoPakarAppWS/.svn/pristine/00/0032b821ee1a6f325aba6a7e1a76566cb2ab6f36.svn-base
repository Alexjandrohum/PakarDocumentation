...
public class InfoTiendaDAO {
    ...
    public int buscaTokenUsuario(String tokenUsuario, Connection con) {
        ...
        ResultSet rs = null;
        String sql = "SELECT id_usuario FROM aps.usuario WITH(NOLOCK) WHERE token_usuario =  ?";
        try {
        ...
        try {
            ...
            while (rs1.next()) {
                sql
                        ...
                        + "FROM aps.tienda_informacion_adicional ia WITH(NOLOCK) \n"
                        + "INNER JOIN aps.tienda_informacion_catalogo ic WITH(NOLOCK) ON ia.id_tienda_informacion_catalogo = ic.id_tienda_informacion_catalogo \n"
                        + "WHERE ia.id_tienda = ? \n"
    ...
    public String actualizaInfoTienda(InfoTiendaDTO dto, Connection con) {
        String sql
                = "UPDATE aps.tienda_informacion_adicional \n"
    ...
    public BufferedImage getImagenTienda(Connection connection, String rutaFotoDefault, String numeroTienda) {
        ...
        String query = 
                  "SELECT \n"
                + "	((SELECT ISNULL(valor,'') FROM aps.propiedad WHERE clave = 'ws_foto_tienda') + CAST(t.numero AS VARCHAR)) + '.jpg' AS url_foto_tienda \n"
    ...
    public String getUrlImagenTienda(Connection connection, String numeroTienda) {
        ...
        try {
            ...
            if (connection != null) {
                ...
                r = ps.executeQuery();
                if (r.next()) {
                    urlImagen = r.getString("url_foto_tienda");
                }
            }
        } catch (Exception ex) {
            Util.registraError(ex);
